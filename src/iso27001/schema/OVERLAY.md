...
The public schema [iso27001.yaml](iso27001.yaml) ships with **paraphrased** descriptions only. The normative text of ISO/IEC 27001:2022 (and Amd. 1:2024) is © ISO/IEC and is not reproduced anywhere in this repository.
...
| --- | ------------------------------------------------------------------ | -------------------- | ------------------- | ------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | [`iso27001.yaml`](iso27001.yaml)                                   | yes                  | yes (Apache-2.0)    | maintainers                          | The public schema: structure (classes, slots, enums, mappings) plus **paraphrased** descriptions. Complete and usable on its own.                                                         |
| 2   | [`iso27001-overlay.template.yaml`](iso27001-overlay.template.yaml) | yes                  | yes (Apache-2.0)    | generated by `just create-empty-overlay` | An empty scaffold listing every class, slot, enum, and permissible value with **empty** `description:` and `comments:` placeholders. No copyrighted text. Read-only - never edit by hand. |
| 3   | `iso27001-overlay.yaml`                                            | **no** - git-ignored | **NO** - local only | **you**, the license holder          | A copy of (2) with the empty placeholders filled in from your licensed copy of the standard. Contains verbatim normative text. Never commit, never publish.                               |

A fourth file, `tmp/iso27001-merged.yaml`, is produced when you run
...
```mermaid
flowchart LR
    T[2 · iso27001-overlay.template.yaml<br/>empty scaffold, public] -.copy.-> O
    P[1 · iso27001.yaml<br/>public schema, paraphrased] --> M{{just overlay-licensed-text<br/>scripts/overlay_populated_text.py}}
    O[3 · iso27001-overlay.yaml<br/>your filled-in copy, private] --> M
    M --> R[tmp/iso27001-merged.yaml<br/>merged, local only]
...
This (re)writes `iso27001-overlay.template.yaml`. It is checked into the
...
The new file `iso27001-overlay.yaml` is git-ignored (see
[.gitignore](../../../.gitignore)) so it cannot accidentally be committed.

### Step 3 - fill in `iso27001-overlay.yaml` from your licensed copy

Open `iso27001-overlay.yaml` in an editor. You will see entries like:
...
For each element you want to enrich, replace the empty `description:` with the verbatim normative text of the corresponding clause from **your own licensed copy** of ISO/IEC 27001:2022. To find the right clause, open the matching element in [iso27001.yaml](iso27001.yaml) and look at its `annotations`:

```yaml
classes:
  InformationSecurityPolicy:
    annotations:
      iso27001_clause: "5.2" # main clause to copy text from
      annex_a_reference: "A.5.1" # optional Annex A control reference
```
...
- **`comments:`** - a list. Use it to add the clause citation (e.g.
  `'source: ISO/IEC 27001:2022 Clause 5.2'`) and any ISO/IEC 27002:2022
  implementation guidance that supplements the main clause. Leave as `[]`
...
This runs `scripts/overlay_populated_text.py` and writes `tmp/iso27001-merged.yaml` - the public schema with your overlay text substituted in. Use this merged file as the input to your own internal
`gen-project`, `gen-doc`, or validation tooling. Do not redistribute it.

### Step 5 - re-sync after upstream schema changes

When the public `iso27001.yaml` changes (new classes, renamed slots,
...
`overlay_populated_text.py` performs a deep merge from `iso27001-overlay.yaml`
onto `iso27001.yaml`, restricted to a fixed allow-list of descriptive
keys:
...
private branch or artefact store; never push `iso27001-overlay.yaml` or
`tmp/iso27001-merged.yaml` to a public remote.
